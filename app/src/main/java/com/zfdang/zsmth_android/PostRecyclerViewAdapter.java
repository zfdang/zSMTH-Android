    ...
    public void inflateContentViewGroup(ViewGroup viewGroup, TextView contentView, final Post post) {
        ...
        for (int i = 1; i < contents.size(); i++) {
            ...
            if (content.getType() == ContentSegment.SEGMENT_IMAGE) {
                ...
                img = (ImageView) inflater.inflate(R.layout.image_post_img, viewGroup, false);
                img.setScaleType(ImageView.ScaleType.CENTER_CROP);
                img.setPadding(3, 3, 3, 3);
                Glide.with(mListener).load(content.getUrl())
                        .thumbnail(0.1f).into(img);
                img.setTag(R.id.image_tag, content.getImgIndex());
