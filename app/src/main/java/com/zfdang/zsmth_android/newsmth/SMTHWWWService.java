...
public interface SMTHWWWService {
  ...
  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/user/ajax_login.json") Observable<AjaxResponse> login(
  ...
  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/user/ajax_logout.json") Observable<AjaxResponse> logout();

  // http://www.newsmth.net/bbsfav.php?select=1
  @GET("/bbsfav.php") Observable<ResponseBody> getFavoriteByPath(@Query("select") String path);

  // http://www.newsmth.net/bbsboa.php?group2=1368
  @GET("/bbsboa.php") Observable<ResponseBody> getBoardsInGroup(@Query("group2") String group2);

  @GET("/nForum/section/{section}?ajax") Observable<ResponseBody> getBoardsBySection(@Path("section") String section);

  // http://www.newsmth.net/nForum/board/FamilyLife?ajax&p=2
  @GET("/nForum/board/{boardEngName}?ajax") Observable<ResponseBody> getBoardTopicsByPage(@Path("boardEngName") String boardEngName,
      @Query("p") String page);

  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/article/{boardEngName}/{topicID}?ajax")
  ...
  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/mainpage?ajax") Observable<ResponseBody> getAllHotTopics();

  // http://www.newsmth.net/nForum/s/article?ajax&t1=ad&au=ad&m=on&a=on&b=WorkLife
  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/s/article?ajax") Observable<ResponseBody> searchTopicInBoard(
  ...
  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/user/query/{username}.json") Observable<UserInfo> queryUserInformation(
      @Path("username") String username);

  @Headers({ "Content-Type: application/octet-stream", "X-Requested-With:XMLHttpRequest" })
  @POST("/nForum/att/{boardEngName}/ajax_add.json") Observable<AjaxResponse> uploadAttachment(@Path("boardEngName") String boardEngName,
      @Query("name") String filename, @Body RequestBody fileContent);

  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/article/{boardEngName}/ajax_post.json")
  ...
  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/article/{boardEngName}/ajax_edit/{postID}.json")
  ...
  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/mail/{mailid}/ajax_send.json")
  ...
  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/user/ajax_session.json") Observable<UserStatus> queryActiveUserStatus();

  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/fav/op/{favid}.json")
  Observable<AjaxResponse> manageFavoriteBoard(@Path("favid") String favid, @Field("ac") String action, @Field("v") String boardEngName);

  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/article/{boardEngName}/ajax_add_like/{topicID}.json")
  ...
  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/article/{boardEngName}/ajax_forward/{postID}.json")
  ...
  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/mail/{folder}?ajax") Observable<ResponseBody> getUserMails(
      @Path("folder") String folder, @Query("p") String page);

  // http://www.newsmth.net/nForum/refer/like?ajax&p=2
  @Headers("X-Requested-With:XMLHttpRequest") @GET("/nForum/refer/{folder}?ajax") Observable<ResponseBody> getReferPosts(
      @Path("folder") String folder, @Query("p") String page);

  // http://www.newsmth.net/nForum/refer/reply/ajax_read.json
  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/refer/{folder}/ajax_read.json")
  ...
  @Headers("X-Requested-With:XMLHttpRequest") @GET("{mail_url}") Observable<AjaxResponse> getMailContent(@Path("mail_url") String mail_url);

  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/friend/ajax_add.json") Observable<AjaxResponse> addFriend(
      @Field("id") String userid);

  // http://www.newsmth.net/bbsdel.php?board=Test&id=910916
  @GET("/bbsdel.php") Observable<ResponseBody> deletePost(@Query("board") String boardEngName, @Query("id") String postID);

  // http://www.newsmth.net/nForum/mail/inbox/ajax_delete.json
  // http://www.newsmth.net/nForum/refer/reply/ajax_delete.json
  // m<mail_id>=on
  @FormUrlEncoded @Headers("X-Requested-With:XMLHttpRequest") @POST("/nForum/{type}/{folder}/ajax_delete.json")
